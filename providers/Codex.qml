...
Item {
  ...
  readonly property var provider: ({
    ...
    settings: [
      {key: "model", type: "string", label: "Model", "default": "gpt-5.6-luna", description: "Must be available in your Codex subscription"},
  ...
  function launch(id, text, cwd) {
    ...
    if (desktop) {
      ...
      argv = ["omarchy-launch-terminal", "codex"]
      if (id) argv.push("resume", id)
      if (cwd) argv.push("--cd", cwd)
      if (!id && text) argv.push("--", text)
