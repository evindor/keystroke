...
F = {
'root': dict(rows=[row('Applications','Every app, one shortcut away','󰀻','Applications'),row('Clipboard History','Text & images, ready to use again','󰅌','Clipboard'),row('Codex','Quick questions, recent conversations and tasks','✳','Codex'),row('Calculator','Try 128 * 1.24 or 15% of 80','󰃬','Calculator'),row('Convert Anything','Units, temperatures & time zones','󰯍','Converter'),row('Search Files','Find files and folders in your home','󰈞','Files'),row('Extensions','Discover, install and manage providers','󰏗','Extensions')]),
...
'extensions':dict(scope='extensions',title='Extensions',provider='extensions',rows=[row('Keystroke Timer','Countdown timers · v1.0.0','󱎫','Installed',accessory='On'),row('Refresh catalog','Keystroke index + Omarchy marketplace','󰑐','Discover'),row('Check for updates','Check installed extensions for new versions','󰚰','Manage')]),
'extension-detail':dict(scope='extensions/timer',title='Keystroke Timer',provider='extensions',rows=[row('Enabled','Use this provider in Keystroke','󰨚','Keystroke Timer',accessory='On'),row('Loaded in omarchy-shell','Omarchy plugin is enabled','󰐊','Keystroke Timer',accessory='On'),row('Settings','Configure the timer provider','󰒓','Keystroke Timer'),row('Check for updates','Check the remote repository','󰚰','Keystroke Timer'),row('Open repository','github.com/evindor/keystroke-timer','󰊤','Keystroke Timer'),row('Remove','Uninstall this extension','󰆴','Keystroke Timer')]),
 'timer':dict(query='timer 25m focus',provider='timer',providerName='Timer',rows=[row('Start 25m timer','focus','󱎫','Timer',tier='answer',badge='plugin',preview='25m',previewLabel='TIMER',previewDetail='focus')]+fallback('timer 25m focus')),
