...
- **Clipboard history** reuses Omarchy's saved text and images. Selecting an entry copies it by default. Keystroke Settings → Clipboard History → **Paste on selection** optionally pastes the selected entry into the previous window; it is off by default. The **Paste shortcut** setting uses `Ctrl+Shift+V` for terminal windows and `Ctrl+V` elsewhere, with `Shift+Insert` available as an override.
