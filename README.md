...
- **Enabled** on an extension's screen asks for confirmation: it says the extension was automatically checked and reviewed before it shipped, that it nonetheless runs at your own risk, and that checking its code first is recommended (**Open source** on the same screen). Confirming loads it at once. Turning it off destroys its service. One that failed to load shows **Needs attention** with the QML error.
